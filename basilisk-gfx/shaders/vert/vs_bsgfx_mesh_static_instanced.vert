...
void main() {
    ...
    mat3 normal_matrix = transpose(inverse(mat3(bsgfx_mesh_instances[instance].transform)));

    out_position = bs_Position;
    out_normal = normalize(normal_matrix * bs_Normal);
    out_texture = vec3(bs_Texture, 0.0);
    out_world_position = bsgfx_translateStaticMesh(instance, bs_Position).xyz;
