...
layout (location = 2) in vec3 bs_Texture;
layout (location = 3) in uint bsgfx_Index;
...
void main() {
    ...
    out_instance = bsgfx_Index;

    gl_Position = bsgfx_process(pushConst.camera * vec4(bs_Position.x, bs_Position.y, bs_Position.z, 1.0));
